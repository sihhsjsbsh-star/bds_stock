            ...
            with col1:
                if st.button("💾 Guardar Cambios", use_container_width=True, type="primary"):
                    try:
                        ...
                        df_editado_reordenado = df_editado[['CATEGORIA', 'MARCA', 'PRODUCTO', 'CONTADO', '12 CUOTAS', '6 CUOTAS', 'STOCK']]
                        guardar_productos(df_editado_reordenado)
    ...
    with tab2:
        ...
        try:
            ...
            if df_ventas.empty:
                ...
                with col1:
                    total_ventas = df_filtrado['MONTO_TOTAL'].sum()
                    st.metric("Total Vendido", f"₲ {total_ventas:,.0f}")
                with col2:
                    cantidad_ventas = len(df_filtrado)
                    st.metric("Cantidad de Ventas", cantidad_ventas)
                with col3:
                    if len(df_filtrado) > 0:
                        promedio = df_filtrado['MONTO_TOTAL'].mean()
